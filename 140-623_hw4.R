...
#' I calculated descriptive statistics and determined that the overall mean and median death rates are `r round(mean(framData$Rate))` and `r round(median(framData$Rate))`. Interestingly, the `Rate` variable is skewed highly to the right indicating that there are outliers with high death rates. The study population is `r round(sum(framData$gender)/length(framData$gender)*100)`% female; out of the total `r length(framData$gender)` study participants, `r sum(framData$gender)` were women and `r length(framData$gender)-sum(framData$gender)` were men. Roughly `r round(sum(framData$bpmeds)/length(framData$bpmeds)*100)`% of the study participants took blood pressure medication. All of the variables in my log-linear model (`r names(coef(model3))[-1]`) were statistically significant (based on an $\alpha$ value of 0.5). The results of the model of summarized in Table 1. The first column shows the death rate ratio, the second and third columns show the lower and upper confidence intervals (respectively) and the final column shows the first five subzero digits of the p-value. All of the death rate ratios were above 1 except for gender indication that being a current smoker, taking anti-hypertensive medication, being diabetic, being obese and being elderly were all associated with a higher death rate, while being female meant that participants were less likely to die in the Framingham study. The `diabetes` variable had the highest coefficient, but also the widest confidence interval.
...
#' In conclusion, this analysis presents a multivariate log-linear model and univariate plots that highlight a potentially important link between various variables and the death rate in the Framingham Heart Study. Among the variables studied (`r names(coef(model3))[-1]`), diabetes stood out as having the strongest association (highest coefficient) with the death rate. The analyses described herein also present the possibility that diabetes could be an important risk factor for cardiovascular disease. Further research is needed to improve our understanding of the interactions and etiologies of diabetes and cardiovascular disease. This work is only the beginning and more precise answers to the research questions discussed in the introduction will require further inspection with models more precisely adapted to each research question. 

knitr::kable(df, format = "markdown")

#' `r tbls(name="adj_RR_tbl", caption = "Adjusted Rate Ratio Estimates of Death obtained from Log-Linear Regression.")`
...
#' `r figs(name="km_plots_fig", caption = "Death Rates per Time Bin in the Framingham Heart Study")`